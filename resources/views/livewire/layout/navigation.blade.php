...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            <div class="flex flex-col items-start">
                                ...
                                @if(auth()->user()->isAdmin() && auth()->user()->current_tenant_id !== auth()->user()->tenant_id)
                                    <div class="text-xs text-gray-500">
                                        Acting as: {{ auth()->user()->currentTenant->name }}
                                    </div>
                                @endif
                            </div>
                    ...
                    <x-slot name="content">
                        ...
                        @if(auth()->user()->isAdmin())
                            <x-dropdown-link :href="route('tenants.select')" wire:navigate>
                                {{ __('Switch Tenant') }}
                            </x-dropdown-link>
                        @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <!-- Responsive Categories Navigation Link -->
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <div class="mt-3 space-y-1">
                ...
                @if(auth()->user()->isAdmin())
                    <x-responsive-nav-link :href="route('tenants.select')" wire:navigate>
                        {{ __('Switch Tenant') }}
                    </x-responsive-nav-link>
                @endif
